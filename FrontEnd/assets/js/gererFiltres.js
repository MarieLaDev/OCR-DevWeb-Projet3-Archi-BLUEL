...
/**
 * Gère les boutons filtres de la page index.html
 * Dans le main, attendra que les fonctions attendreDOM et genererGalerie 
 * soient terminées. Cette fonction appelle la fonction genererFiltres()
 * présente en bas du code
 */
export async function gererFiltres() {
     try {
          // générer d'abord les boutons avec la fonction genererBtnsFiltres puis...
          attendreDOM(genererBtnsFiltres); 
          ...
          for (let filtre of btnsFiltres) {
               ...
               filtre.addEventListener("click", (event) => {
                    ...
                    clickedBtn.classList.add("focus-btn"); 
...
async function genererBtnsFiltres() {
     ...
     // Déclare le tableau catégorie
     let categoriesGalerie = [];
     
     // Déclare le new Set() qui contiendra toutes les nouvelles catégories mais exclura les autres
     let ajoutCategorie = new Set();

     // Récupérer les catégories des projets
     ...
     for (let categorie of listeCategories) {
          ...
          genereFiltre.className = "btn";
          
          if (categorie.id === 0) {
               genereFiltre.className = "focus-btn";
          } else {
               genereFiltre.className = "btn";
          }
